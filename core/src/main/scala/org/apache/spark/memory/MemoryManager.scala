...
private[spark] abstract class MemoryManager(
  ...
  // edit by Eddie
  // change val to var
  protected[this] var maxOffHeapMemory = conf.getSizeAsBytes("spark.memory.offHeap.size", 0)
  protected[this] var offHeapStorageMemory =
  ...
  // edit by Eddie
  // Test info
  logInfo("MemoryManager is started. " +
  "on-heap storage memory is:" + onHeapStorageMemory.toString() +
  " on-heap execution memory is: " + onHeapExecutionMemory.toString() +
  ...
  // Edit by Eddie
  def stopUpdateMemoryThread(): Unit = {}
}
